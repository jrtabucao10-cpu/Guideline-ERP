...
const originalPrintableDocument=printableDocument;
printableDocument=function(type,id){
  if(type!=='pr')return originalPrintableDocument(type,id);
  const r=pr(id),s=supplier(r.supplierId),p=project(r.projectId),j=job(r.jobOrderId),fallbackMaterial=material(r.materialId);
  const lines=(r.lines&&r.lines.length?r.lines:[{code:fallbackMaterial.code||'',description:r.item||fallbackMaterial.description||'',qty:'',unit:fallbackMaterial.unit||'',remarks:s.name||r.supplier||''}]);
  const scopeBoxes=['JOINERY 1','INSTALLATION','PAINT','JOINERY 2','GYPSUM','CNC','EXT. FAB.'].map(x=>`<label>${x}<span></span></label>`).join('');
  const itemRows=Array.from({length:20},(_,i)=>{const l=lines[i]||{};return `<tr><td>${i+1}</td><td colspan="3">${l.code?`<b>${l.code}</b> - `:''}${l.description||''}</td><td>${l.qty||''}</td><td>${l.unit||''}</td><td>${l.remarks||''}</td><td></td></tr>`}).join('');
  const html=formShell(`<table class="form-table pr-layout-table"><tbody><tr><td rowspan="4" class="side-title"><h2>PURCHASE<br>REQUEST</h2>${formLogo()}</td><th>Project:</th><td colspan="2">${p.name||'—'}</td><th>Project #:</th><td>${p.no||'—'}</td><th>PR #:</th><td>${r.no||''}</td></tr><tr><th>BOQ Item #:</th><td>${r.boqItemNo||''}</td><th>Job Order #:</th><td>${j.no||'—'}</td><th>Issue Date:</th><td>${r.date||''}</td></tr><tr><td colspan="7" class="checkbox-row">${scopeBoxes}<b>Duration Estimate<br>${r.durationDays||'____'} days</b></td></tr><tr><th>Requested By:</th><td>${r.requestedBy||''}</td><th>Project Manager Engineer:</th><td>${r.projectManager||''}</td><th>Store Manager:</th><td>${r.storeManager||''}</td><th>No. of Labors</th><td>${r.noOfLabors||''}</td></tr><tr><th>Item #</th><th colspan="3">Description</th><th>Qty.</th><th>Unit</th><th>Remarks</th><th>Store Availability</th></tr>${itemRows}<tr><th colspan="8">Projects Manager</th></tr><tr><td>Shop Dwg. approval</td><td colspan="4">Comments<br>${r.comments||''}</td><td colspan="3">Signature:<br><br>Date: / /</td></tr><tr><td>Variation approval</td><td colspan="4"></td><td colspan="3"></td></tr><tr><th colspan="8">Technical Director</th></tr><tr><td colspan="5">Comments</td><td colspan="3">Signature:<br><br>Date: / /</td></tr><tr><td colspan="4">Delivered on: / /</td><td colspan="4">Received by: __________ Signature: __________</td></tr></tbody></table>`,'pr-layout pr-word-format');
  let holder=$('#printDocument');if(!holder){holder=document.createElement('div');holder.id='printDocument';document.body.appendChild(holder)}
  holder.innerHTML=html;document.body.classList.add('show-print-doc');$('#closePrintDoc').onclick=()=>document.body.classList.remove('show-print-doc');$('#printNow').onclick=()=>window.print();
}
init();
