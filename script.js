const KEY='guideline-fitout-erp-v2';
...
const baseNav=[['dashboard','▣','Dashboard'],['quotations','Q','Quotations'],['projects','P','Projects'],['joborders','J','Job orders'],['requests','R','Purchase requests'],['purchases','L','LPO / purchases'],['labour','H','Labour costing'],['reports','▤','Cost reports']];
const activeNav=()=>baseNav.filter(n=>company().hasJobOrders||n[0]!=='joborders');
function init(){
  $('#today').innerHTML=companyToggle();
  drawNav();
  $('#resetDemo').onclick=()=>{if(confirm('Start fresh? This removes demo quotations, projects, job orders, purchases, labour and expenses.')){db={quotations:[],projects:[],jobOrders:[],purchaseRequests:[],purchases:[],labour:[],expenses:[]};save();render();toast('Fresh ERP ready.')}};
  render();
}
function drawNav(){
...
function render(){
  ...
  $('#app').innerHTML=companyBanner()+({dashboard,quotations,projects,joborders,requests,purchases,labour,reports})[view]();
  bindForms();
}
...
function requestTable(rows){return rows.length?`<table><thead><tr><th>PR</th><th>${company().hasJobOrders?'Job order':'Project'}</th><th>Supplier</th><th>Items</th><th>Amount</th><th>Status</th></tr></thead><tbody>${rows.map(r=>`<tr><td><b>${r.no}</b><br><small>${r.date}</small></td><td>${company().hasJobOrders?(job(r.jobOrderId).no||'—'):(project(r.projectId).name||'—')}</td><td>${r.supplier||'—'}</td><td>${r.item}</td><td class="money">${money(r.amount)}</td><td><span class="badge ${r.status.toLowerCase()}">${r.status}</span></td></tr>`).join('')}</tbody></table>`:'<p class="empty">No purchase requests yet.</p>'}
...
function bindForms(){
  ...
  bind('#quotationForm',d=>db.quotations.unshift({...d,id:uid('q'),companyId:currentCompany,amount:+d.amount}));
  bind('#projectForm',d=>db.projects.unshift({...d,id:uid('p'),companyId:currentCompany,budget:+d.budget}));
  ...
  bind('#prForm',d=>{const projectId=company().hasJobOrders?job(d.jobOrderId).projectId:d.projectId;db.purchaseRequests.unshift({...d,id:uid('pr'),companyId:currentCompany,projectId,jobOrderId:d.jobOrderId||'',amount:+d.amount})});
  bind('#purchaseForm',d=>{const projectId=company().hasJobOrders?job(d.jobOrderId).projectId:d.projectId;db.purchases.unshift({...d,id:uid('po'),companyId:currentCompany,projectId,jobOrderId:d.jobOrderId||'',amount:+d.amount})});
  ...
  document.querySelectorAll('[data-create-project]').forEach(b=>b.onclick=()=>{const q=quotation(b.dataset.createProject);db.projects.unshift({id:uid('p'),companyId:currentCompany,quotationId:q.id,no:company().prefix+'-P-'+Date.now().toString().slice(-4),name:q.scope.slice(0,45),client:q.client,start:today(),status:'Active',budget:+q.amount||0});save();view='projects';render();toast('Project created from approved quotation')});
